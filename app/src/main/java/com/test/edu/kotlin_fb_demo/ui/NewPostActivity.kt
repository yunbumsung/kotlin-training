...
import com.test.edu.kotlin_fb_demo.R.id.fab
import com.test.edu.kotlin_fb_demo.R.id.toolbar
...
class NewPostActivity : RootActivity() {
    ...
    private fun uploadNewPost (user:User) {
        val mDataBase = FirebaseDatabase.getInstance().getReference()
        // 글 입력
        // 중복되지 않는 해시키: 해당글의 고유키
        // 전체글: /posts/중복되지 않는 해시키/POST덩어리
        // 내글: /user-posts/사용자아이디/중복되지 않는 해시키/POST덩어리
        val uid = userID()
    ...
    private fun editEnable(enable:Boolean) {
        // 데이터 베이스에 글 업로드 중에는 사용자가 에디팅 하거나, 전송을 더블로 요청하는 것을 막는다
        // 에디터는 비활성
